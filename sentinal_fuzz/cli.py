...
def scan(
    ...
    auth_cookie: str | None = typer.Option(
        None, "--auth-cookie",
...
def crawl(
    ...
    async def _run_crawl() -> None:
        ...
        async with __import__("sentinal_fuzz.utils.http", fromlist=["HttpClient"]).HttpClient(
            ...
            endpoints = await scanner._crawler.crawl()
...
def _validate_template(data: object) -> list[str]:
    ...
    if is_flat:
        ...
        if "payloads" in data and not isinstance(data["payloads"], (list, str)):
...
def template_new(
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        ...
        f.write("# Generated by sentinal-fuzz template new\n\n")
